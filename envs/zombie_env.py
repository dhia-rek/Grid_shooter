"""
zombie_env.py — Staged Zombie Shooter (no timer, death-based episodes).

Stages advance by kill count. Last stage is infinite max-difficulty.
Episode ends only when a zombie hits the agent.

Stage 1 — Recruit   : kill  5 → advance
Stage 2 — Soldier   : kill 15 → advance
Stage 3 — Veteran   : kill 30 → advance
Stage 4 — INFINITE  : endless, max speed, high score mode
"""
...
class ZombieEnv(gym.Env):
    ...
    def __init__(self, grid_size=GRID, max_steps=MAX_STEPS):
        ...
        # obs: agent(2) + bullet(3) + zombies(MAX_Z*3) + stage(1) + kills_norm(1)
        obs_dim = 2 + 3 + MAX_Z * 3 + 1 + 1
        ...
        if self.bullet is not None:
            ...
            for z in self.zombies:
                if z[2] and z[0] == self.bullet[0] and z[1] == self.bullet[1]:
                    z[2] = 0
                    self.kills += 1
                    kill_bonus = 10.0 + self.stage * 5.0   # higher stages = bigger reward
                    reward += kill_bonus
                    self.bullet = None
                    break
    ...
    def _spawn_zombie(self):
        x = int(self.np_random.integers(0, self.G))
        ax, ay = self.agent_pos
        if ay == 0 and x == ax:
            x = (x + 1) % self.G
        self.zombies.append([x, 0, 1])
    ...
    def _obs(self):
        ...
        alive = sorted(self.zombies,
                       key=lambda z: abs(z[0]-self.agent_pos[0]) + abs(z[1]-self.agent_pos[1]))
        slots = (alive + [[0,0,0]] * MAX_Z)[:MAX_Z]
        zobs  = [v for z in slots for v in (z[0]/g, z[1]/g, float(z[2]))]
